...
int main(int argc, char **argv) {
    ...
    strcpy(users[1].username, "test");
    strcpy(users[1].password, "qwerty");

    strcpy(users[2].username, "emil");
    strcpy(users[2].password, "379");
    ...
    while (1) {
        ...
        if ((pid = fork()) == 0) {
            ...
            do {
                User user;
                if ((x = read(connfd, &user, sizeof(User))) < 0) {
                    break;
                }
                ...
                char message [20];
                if (isFound == 1) {
                    strcpy(message, "logged in");
                } else {
                    strcpy(message, "no user");
                    printf("NOT DETECTED!\n");
                }
                encrypt(message);
                write(connfd, message, 20);
            } while (isFound != 1);
