...
int get_command(char *command) {
    ...
    while (check < 0) {
        ...
        if ((strcmp(str, "ls") == 0) || (strcmp(str, "get") == 0) || (strcmp(str, "put") == 0) ||
            (strcmp(str, "quit") == 0)) {
            check = 1;
...
int main(int argc, char **argv) {
    ...
    do {
        ...
        write(controlfd, &user, sizeof(User));


        read(controlfd, isLoggedIn, 20);
        decrypt(isLoggedIn);
        printf(isLoggedIn);
        printf("\n");
        if (strcmp(isLoggedIn, "logged in") == 0) {
            printf("no error\n");
        } else if (strcmp(isLoggedIn, "no user") == 0) {
            printf("error\n");
        }
        scanf("%c");//fix one char problem
    } while (strcmp(isLoggedIn, "logged in") != 0);
